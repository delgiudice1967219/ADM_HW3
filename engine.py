...
def find_restaurants(query, vocabulary_df, inverted_index, df):
    ...
    try:  # Try finding restaurants
        # Retrieve the term_ids for each token in the query (if a query term is not in the vocabulary,
        # it raises an error, handled in the 'except' block)
...
def top_k_restaurants(query, inverted_index, vocabulary_dict, doc_tf_idf_scores, df, n, k=5):
    ...
    docs_to_consider = []  # initialize list to store documents to consider
    # (non-zero intersection with the query tokens)
    ...
    def query_doc_intersection(query_tf_idf_scores, doc_tf_idf_scores):
        ...
        while i < len(query_tf_idf_scores) and j < len(doc_tf_idf_scores):  # if they have the same term,
    ...
    for doc_id in docs_to_consider:
        query_intersection, doc_intersection = query_doc_intersection(query_tf_idf_scores,
                                                                      doc_tf_idf_scores[doc_id]) # find the intersection
        # of terms in query and doc
...
def get_weighted_similarity(df, queries, fields):
    ...
    Inputs:
    df: DataFrame with restaurants data
    queries: dictionary with query info for each field
    fields: list of the fileds to consider

    Output:
    array with weighted cosine similarity scores
    '''
    # Check tha active textual fields
    ...
    for field in active_fields:
        similarity_scores = top_cosine_similarity(df[field], queries[field])
...
def enanched_search_restaurants(df, search_params):
    try:
        # Collect the input text fields
        queries = {
            "restaurantName": search_params["restaurant_name"].strip().lower(),
            "city": search_params["city"].strip().lower(),
            "cuisineType": search_params["cuisine_type"].strip().lower()
        }
        ...
        # Create a temp DataFrame with the similarity scores
        results = df.copy()
        results['similarity_score'] = similarity_scores

        # Consider only the restaurants with similarity score larger than 0
        if any(queries.values()):
            results = results[results['similarity_score'] > 0.0]

        # Collect filters values
        selected_price_range = search_params["price_range"]
        selected_regions = search_params["regions"]
        selected_credit_cards = search_params["credit_cards"]
        selected_facilities = search_params["facilities"]

        # Apply the filtering
        # Filter for Price Range interval
        if selected_price_range:
            results = results[results["priceRange"].between(
        ...
        # Filter for Facilities & Services
        ...
        # Order by cosine similarity score
        if any(queries.values()):
